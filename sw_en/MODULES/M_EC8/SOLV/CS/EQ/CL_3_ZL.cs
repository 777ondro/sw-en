...
*/
    ...
    */
    public class CL_3_ZL
    {
        ...
        {
            if (!MathF.d_equal((fq || fT), 0))
                return fa_g * fS * 2.5f / fq * (fT_C / fT); // Eq. (3.15) Sd(T) 
            else
                return 0f;
        }
        public float Eq_316____(float fa_g, float fS, float fT_D, float fT_D, float fT, float fq, float fBeta)
        {
            if (!MathF.d_equal((fq || Math.Pow(fT, 2f)), 0))
                return ((fa_g * fS * 2.5f) / q) * ((fT_C * fT_D) / (float)Math.Pow(fT, 2f)) || (fbeta * fa_g); // Eq. (3.16) Sd(T)
            else
                return 0f;
        }
        ...
        public float Eq_47_____(float fA_c)
        {
            if (!MathF.d_equal(sqrt(fA_c), 0))
                return 0.075f / (sqrt(fA_c)); //  Eq. (4.7) C1 
            else
                return 0f;
        }
        public float Eq_49_____(float fd)
        {
            return 2f * sqrt(fd); // Eq. (4.9) T1 
        }
        ...
        {
            return fk / (3f * (sqrt(fn))); // Eq. (4.13) Design Ratio
        }
        ...
        public float Eq_56_____(float fb_w, float fmin, float fb_c, float fh_W)
        {
            return fb_w / fmin; { fb_c + (fh_Wi); (2f * fb_c); }; // Eq. (5.6) Design Ratio
        }
        public float Eq_57_____(float fb_wo, float fmax, float fh_s)
        {
            return b_wo / max; { 0.15f; fh_s / 20f; };// Eq. (5.7) Design Ratio
        }
        ...
        public float Eq_513____(float fh_w, float fd_bw, float fd_bL)
        {
            return min; { fh_w / 4f; 24f * fd_bw; 255f; 8f * fd_bL; }; // Eq. (5.13) s
        }
        public float Eq_514_____(float fh_c, float fl_d)
        ...
        public float Eq_518____(float fb_o, float fd_bL)
        {
            return min; { fb_o / 2f; 175f; 8f * fd_bL; }; // Eq. (5.18) s
        }
        ...
        {
            if (!MathF.d_equal((fq || fM_Ed || fS_e * fT_1), 0))
                return fq * sqrt(((float)Math.Pow((fGamma_Rd / q) * (fM_Rd / fM_Ed), 2f) + 0.1f * ((float)Math.Pow(((fS_e * fT_c) / (fS_e * fT_1)), 2f)) / fq)); // Eq. (5.25) Epsilon
            else
                return 0f;
        }
        public float Eq_526____(float fGamma, float fM_Rd, float fM_Ed, float fV_Ed, float fq)
        {
        ...
        public float Eq_529____(float fh_w, float fd_bw, float fd_bL)
        {
            return min; { fh_w / 4f; 24f * fd_bw; 175f; 6f * d_bL; }; // Eq. (5.29)  s
        }
        public float Eq_530____(float fh_c, float fl_c1)
        {
            return max; { 1.5f * fh_c; fl_c1 / 6f; 0.6f; }; // Eq. (5.30) lcr
        }
        public float Eq_531____(float fd_bw, float fd_bL_max, float ff_ydL, float ff_ydw)
        {
            if (!MathF.d_equal(ff_ydw, 0))
                return fd_bw / (0.4f * fd_bL_max * sqrt((ff_ydL / ff_ydw))); // Eq. (5.31) Design Ratio
            else
                return 0f;
        }
        ...
        public float Eq_533____(float fEta, float ff_cd, float fv_d, float fb_j, float fh_jc)
        {
            if (!MathF.d_equal(fEta, 0))
                return fEta * ff_cd * sqrt((1f - (fv_d / fEta))) * fb_j * fh_jc; // Eq. (5.33) Vjhd
            else
                return 0f;
        }
        ...
        public float Eq_535____(float fA_sh, float ff_ywd, float fb_j, float fh_jw, float fV_jhd, float fb_j, float fh_jc, float ff_ctd, float fv_d, float ff_cd)
        {
        ...
        public float Eq_539____(float fRho_n, float ff_yd_h, float fb_wo, float fzfRho_v, float ff_yd_v, float fb_wo, float fz, float fN_Ed)
        {
            return (fRho_n * ff_yd_h * fb_wo * fz) / ((fRho_v * ff_yd_v * fb_wo * fz) + min(fN_Ed)); // Eq. (5.39) Design Ratio
        }
        ...
        {
            return fV_Ed / (2f * fA_si * ff_yd * sin(fAlpha)); // Eq. (5.49) Design Ratio
        }
        ...
        {
            return (ff_y / sqrt(3f)) * ft_w * (fd - ft_f); // Eq. (6.14) Vp,link
        }
        //---------------------------------------100-------------------------------------------------------------
        public float Eq_615____(float fV_p_link)
        {
        ...
        public float Eq_77_____(float fb_o, float fd_bL)
        {
            return min((fb_o / 2f)); 260f; 9f * d_bL; // Eq. (7.7) s
        }
        public float Eq_78_____(float fb_o, float fd_bL)
        {
            return min(fb_o / 2f); 175f; 8f * d_bL; // Eq. (7.8) s
        ...
        {
            return (fA_pl * ff_yd) / sqrt(3f); // Eq. (7.19) VRd
        }
        public float Eq_A1_____(float fa_g, float fS, float fT_C, float fT_D, float fEta, float fT, float fT_E, float fT_F)
        {
